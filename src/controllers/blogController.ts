...
export const createBlog = async (req: AuthRequest, res: Response) => {
  try {
    ...
    const { title, summary, estimatedReadTime, slug, metaTitle, metaDescription,website,coverImageAlt } = req.body;
    if (!title) return res.status(400).json({ message: "Title is required" });
    ...
    if (blocks.length && images.length) {
      ...
      blocks = blocks.map((block) => {
        if (block.type === "image" && imageIndex < images.length) {
          block.data.image = {
            ...images[imageIndex],
            alt: block.data.alt || "" // Takes 'alt' string from the block data
          };
          imageIndex++;
        }
        return block;
...
export const updateBlogById = async (req: AuthRequest, res: Response) => {
  try {
    ...
    const { title, summary, estimatedReadTime, slug, metaTitle, metaDescription, coverImageAlt } = req.body;
    ...
    if (coverFile) {
      coverImage = {
        data: fs.readFileSync(coverFile.path),
        contentType: coverFile.mimetype,
        alt: coverImageAlt || existingBlog.coverImage?.alt || ""
      };
    } else if (coverImageAlt !== undefined) {
      // If only alt changed but not the file
      if (coverImage) coverImage.alt = coverImageAlt;
    }
    ...
    if (blocks.length && images.length) {
      ...
      blocks = blocks.map((block) => {
        if (block.type === "image" && imageIndex < images.length) {
          block.data.image = { ...images[imageIndex], alt: block.data.alt || "" };
          imageIndex++;
        }
        return block;
      });
    ...
    const updatedBlog = await Blog.findByIdAndUpdate(blogId, {
      title, summary, metaTitle, metaDescription, estimatedReadTime,
      blocks, coverImage, tags, categories, faqs, slug: existingBlog.slug
    }, { new: true });

    return res.status(200).json({ message: "Updated", blog: updatedBlog });
  } catch (err: any) {
    return res.status(500).json({ message: err.message });
  }
};
